...
import com.gmail.volodymyrdotsenko.jqxwicket.core.settings.JQueryLibrarySettings;
...
public abstract class JQueryAbstractBehavior extends Behavior {
	...
	public static JQueryLibrarySettings getJQueryLibrarySettings() {
		if (Application.exists()
			...
			return (JQueryLibrarySettings) Application.get()
	 ...
	 * 
	 * @param name
	 *            the name of the behavior. It is used in the token so the
	 *            behavior can be identified in the generated page.
	 */
	...
	/**
	 * Adds a reference to be rendered at
	 * {@link #renderHead(Component, IHeaderResponse)} time.
	 * 
	 * @param reference
	 *            a {@link CssResourceReference}, a
	 *            {@link JavaScriptResourceReference} or a
	 *            {@link JQueryPluginResourceReference}
	...
	public void renderHead(Component component, IHeaderResponse response) {
		...
		if (settings != null && settings.getJQueryGlobalizeReference() != null) {
			response.render(new PriorityHeaderItem(JavaScriptHeaderItem
					.forReference(settings.getJQueryGlobalizeReference())));
		}
		...
		for (ResourceReference reference : this.references) {
			if (reference instanceof CssResourceReference) {
				response.render(new PriorityHeaderItem(CssHeaderItem
		...
		if (target != null) {
			...
			this.renderScript(
					JavaScriptHeaderItem.forScript(this.toString(),
	 ...
	 * 
	 * @param script
	 *            the {@link JavaScriptHeaderItem}
	 * @param response
	 *            the {@link IHeaderResponse}
	 */
	protected void renderScript(JavaScriptHeaderItem script,
